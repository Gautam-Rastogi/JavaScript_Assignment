const movies = [
  ...
  },
  {
    ...
    year: 1994,
    imdbRating: 9.3,
    actors: ["Tim Robbins", "Morgan Freeman", "Christian Bale"],
  },
  {
    title: "Pulp Fiction",
    genre: "Crime",
...
let movieTitlesAndRatings = movies.map(movie=>("Title : " + movie.title + " , "+ "IMDB Rating : " + movie.imdbRating));
console.log(movieTitlesAndRatings);

// Filter the movies to create an array of thriller movies released after the year 2000.

let thrillerMovies = movies.filter(movie=>movie.genre == "Thriller" && movie.year > 2000).map(movie=>movie.title);
console.log(thrillerMovies);

// Filter the movies to create an array of drama movies with IMDb ratings above 8.5.

let dramaMovies = movies.filter(movie=>movie.genre == "Drama" && movie.imdbRating > 8.5).map(movie=>movie.title);
console.log(dramaMovies);

// Filter the movies to create an array of action movies starring Leonardo DiCaprio.

let actionMovies = movies.filter(movie=>movie.genre == "Action" && movie.actors.includes("Leonardo DiCaprio")).map(movie=>movie.title);
console.log(actionMovies);

// Chain operations to find all drama movies featuring Christian Bale.

let DramaChristian = movies.filter(movie=>movie.genre ==="Drama" && movie.actors.includes("Christian Bale")).map(movie=>movie.title);
console.log(DramaChristian);

//  Chain operations to find all drama movies featuring Tim Robbins.

let dramaTim = movies.filter(movie=> movie.genre === "Drama" && movie.actors.includes("Tim Robbins")).map(movie=>movie.title);
